...
def read_token() -> str:
    if not TOKEN_FILE.exists():
        raise FileNotFoundError(
            "token.txt がありません。bot.py と同じフォルダに作成してください。"
        )

    token = TOKEN_FILE.read_text(encoding="utf-8").strip()

    if not token:
        raise ValueError("token.txt が空です。Botトークンを入力してください。")
...
class LotterySetupView(discord.ui.View):
    ...
    async def start_button(
        ...
        if self.winner_count > len(self.participants):
            await interaction.response.send_message(
                "当選人数が参加者数を超えています。",
                ephemeral=True,
            )
        ...
        winners = random.sample(self.participants, self.winner_count)
